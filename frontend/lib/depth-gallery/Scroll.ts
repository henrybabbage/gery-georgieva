...
export class Scroll {
  ...
  restoreScrollPosition(value: number): void {
    if (!this.isInitialized || !Number.isFinite(value)) return
    ...
    if (this.useScrollBounds) {
      ...
      clamped = THREE.MathUtils.clamp(value, minimumScroll, maximumScroll)
    }

    this.scrollTarget = clamped
    this.scrollCurrent = clamped
    this.previousScrollCurrent = clamped
    this.rawVelocity = 0
    this.velocity = 0

    const nextCameraZ = this.cameraZFromScroll(clamped)
    if (this.useScrollBounds) {
