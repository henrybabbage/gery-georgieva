...
export interface EngineCreateOptions {
  ...
  /** Virtual scroll amount matching `Scroll.getScrollPosition()` after init. */
  initialScrollPosition?: number
  /** Called each frame with current virtual scroll position. */
...
export class Engine {
  ...
  async init(): Promise<void> {
    ...
    try {
      ...
      this.resize()
      const savedScroll =
        this.initialScrollPosition !== undefined && Number.isFinite(this.initialScrollPosition)
          ? this.initialScrollPosition
          : undefined
      if (savedScroll !== undefined) {
        this.scroll.restoreScrollPosition(savedScroll)
      }
  ...
  update(): void {
    ...
    this.scroll.update()
    this.onScrollPositionChange?.(this.scroll.getScrollPosition())
